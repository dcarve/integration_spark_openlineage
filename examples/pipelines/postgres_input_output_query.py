...
builder = (
    SparkSession.builder.appName(f's-postgresinputoutputquery-{taskname}')
...
(
	with A as (
	select team1 as team from staging.example
	union 
	select team2 as team from staging.example
)

select distinct team from A
)
...
(
    ...
    .option("url", f"jdbc:postgresql://{POSTGRES_HOST}:{POSTGRES_PORT}/{POSTGRES_DATABASE}")
    .option("driver", "org.postgresql.Driver")
    .option("dbtable", "staging.example_output_query")
    .option("user", f"{POSTGRES_USER}")
    .option("password", f"{POSTGRES_PASSWORD}")
    .save()
)
