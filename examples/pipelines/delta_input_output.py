...
builder = (
    SparkSession.builder.appName(f's-deltainputoutput-{taskname}')
...
df = spark.read.format('delta').load('examples/data/inputs/delta_example')

df.write.format('delta').mode('overwrite').option("overwriteSchema", "true").save(
    'examples/data/outputs/delta_input_output'
)
